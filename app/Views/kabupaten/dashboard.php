...
<head>
    ...
    <meta name="title" content="DPMD | Dashboard v3" />
    ...
    <div class="app-wrapper">
            ...
            <div class="container-fluid">
                ...
                <ul class="navbar-nav ms-auto">
                    ...
                    <li class="nav-item dropdown user-menu">
                        ...
                        <a href="<?= base_url('auth/logout') ?>" class="btn btn-secondary btn-flat float-end">Sign
                            out</a>
        ...
        <aside class="app-sidebar bg-body-secondary shadow" data-bs-theme="dark">
            ...
            <div class="sidebar-brand">
                <!--begin::Brand Link-->
                <!--begin::Brand Image-->
                <img src="<?= base_url('assets/adminlte/img/') ?>smd.png" alt="Kabupaten Sumedang Logo"
                    class="brand-image opacity-75 shadow" style="width: 40px;" />
                <!--end::Brand Image-->
                <!--begin::Brand Text-->
        ...
        $.ajax({
            ...
            success: function(res) {
                ...
                res.data.forEach(function(item) {
                    ...
                    } else if (item.status_kecamatan === "submitted" || item.status_kecamatan ===
                        "resubmitted") {
                    ...
                    let statusText = item.status_kecamatan ?
                        item.status_kecamatan.charAt(0).toUpperCase() + item.status_kecamatan.slice(
                            1) :
                        "";
    ...
    function modalKecamatan() {
        $(document).on("click", ".btnUploadKecamatan", function() {
            ...
            $.get(`/templates/kecamatan/detail/${id}/${template}/${desa}/`, function(res) {
                ...
                let filesHtml =
                    "<table class='table table-bordered'><thead><tr><th>Upload Desa</th></tr></thead><tbody>";
                res.files.forEach(f => {
                    ...
                    const linkUrl = isPdf ? `${base_url + cleanFilePath}` :
                        `${base_url + cleanFilePath}`;
                    filesHtml += `
            <tr>
            ...
            $.ajax({
                ...
                success: function(res) {
                    if (res.submission) {
                        ...
                        let filesHtml =
                            "<table class='table table-bordered'><thead><tr><th>Document upload</th></tr></thead><tbody>";
                        res.files.forEach(f => {
                            const isPdf = f.file_path.toLowerCase().endsWith('.pdf');
                            // Replace spaces in file_path and file_name for cleaner URLs
                            const cleanFilePath = f.file_path.replace(/\s+/g, '_');
                            ...
                            const linkUrl = isPdf ?
                                `${base_url + cleanFilePath}${cacheBuster}` :
                                `${base_url + cleanFilePath}${cacheBuster}`;
                            filesHtml += `
            <tr>
              <td><a href="${linkUrl}" target="_blank" rel="noopener noreferrer">${cleanFileName}</a></td>
    ...
    $(document).on("submit", "#modalUploadKabupaten form", function(e) {
        ...
        if (status === "rejected" && !keterangan) {
            ...
            if ($("#exampleTextarea").next(".invalid-feedback").length === 0) {
                $("#exampleTextarea").after(
                    '<div class="invalid-feedback">Keterangan wajib diisi jika status rejected.</div>');
            }
        } else {
...
`;
            ...
            `;
                ...
                const modalRiwayat = new bootstrap.Modal(
                    document.getElementById('modalRiwayat')
                );
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    if (res.submission) {
                        $("#uploadDesaTitle").text(res.submission.title);
                        $("#id_template").val(template);
                        $("#id_submission").val(res.submission.id);
                        $("#desa_id").val(desa);
                        $("#earmarked").val(formatRupiahModal(res.submission.earmarked));
                        $("#non_earmarked").val(formatRupiahModal(res.submission.non_earmarked));
                        $("#status_kecamatan").val(res.submission.status_kabupaten);
                        $("#keterangan").val(res.submission.keterangan_kecamatan);
                        $("#status_pengejuan").val(res.submission.keterangan_kabupaten);


                        if (
                            res.submission.keterangan_kabupaten === "SP2D" ||
                            res.submission.keterangan_kabupaten === "Pengajuan KPPN"
                        ) {
                            $("#status_kecamatan_new").prop("disabled", true).val("");
                        } else {
                            $("#status_kecamatan_new").prop("disabled", false);
                        }

                        if (
                            res.submission.status_kecamatan === "submitted"
                        ) {
                            $("#status_pengejuan").prop("disabled", true).val("");
                        } else {
                            $("#status_pengejuan").prop("disabled", false);
                        }
    ...
    <script>
    $(document).on("change", "#status_kecamatan_new", function() {
        if ($(this).val() !== "") {
            $("#status_pengejuan").val(""); // reset select lain
        }
    });

    $(document).on("change", "#status_pengejuan", function() {
        if ($(this).val() !== "") {
            $("#status_kecamatan_new").val(""); // reset select lain
        }
    });
    </script>
